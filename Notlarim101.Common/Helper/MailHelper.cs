   ...
   public class MailHelper
    {
        ...
        public static bool SendMail(string body, List<string> to, string subject, bool isHtml = true)
        {
            ...
            try
            {
                ...
                message.From = new MailAddress(ConfingHelper.Get<string>("MailUser"));
                to.ForEach(x =>
                {
                ...
                using (var smtp=new SmtpClient(
                    ConfingHelper.Get<string>("MailHost"),
                    ConfingHelper.Get<int>("MailPort")))
                {
                    smtp.EnableSsl = true;
                    smtp.Credentials = new NetworkCredential(
                        ConfingHelper.Get<string>("MailUser"),
                        ConfingHelper.Get<string>("MailPass"));
                    smtp.Send(message);
                    result = true;
                }
            }
